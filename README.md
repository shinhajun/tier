...
- 간결한 티어표 목록과 카테고리 필터
- 점수형·문자형·자유형 행 구성
- 수정 화면의 각 행에서 새 항목 생성, 새 행 생성, 점수/메모, 행 간 이동, 순서 변경
...
표별 수정 키는 해당 브라우저의 localStorage에만 편의를 위해 보관하고, Supabase private 스키마에는 SHA-256 전처리 후 bcrypt한 해시만 저장합니다. 전체 표를 복구할 수 있는 관리자 키 원문은 Obsidian 자격증명 노트와 ignored `.env.admin.local`에만 두며 Vite/GitHub secret에는 넣지 않습니다.
